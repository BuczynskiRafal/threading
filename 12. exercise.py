"""Exercise containing abort thread example."""
import datetime
import colorama
import random
import time
...
def main():
    ...
    while any([t.is_alive() for t in threads]):
        ...
        if not abort_thread.is_alive():
            print(colorama.Fore.GREEN + f"Cancelling on your request!", flush=True)
            break

    dt = datetime.datetime.now() - t0
    print(colorama.Fore.BLUE + f"App exiting, total time: {dt.total_seconds():,.2f} sec.", flush=True)


def check_cancel():
    print(colorama.Fore.RED + f"Press enter to cancel...", flush=True)
    input()
